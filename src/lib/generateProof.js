...
export function proofToCalldata(proof) {
  // The verifier expects a uint256[24] array.
  // We need to extract the components from the snarkjs proof object
  // and format them as strings suitable for uint256 in Solidity.
  return [
    proof.A[0], proof.A[1],
    proof.B[0], proof.B[1],
    proof.C[0], proof.C[1],
    ...
    proof.eval_s2,
    proof.eval_zw
  ].map(x => x.toString()); // Ensure all are strings
}
